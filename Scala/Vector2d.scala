...
case class Vector2d (x: Int, y: Int):
    ...
    def lowerLeft(other: Vector2d): Vector2d = new Vector2d(Math.min(x, other.x), Math.min(y, other.y))
    def upperRight(other: Vector2d): Vector2d = new Vector2d(Math.max(x, other.x), Math.max(y, other.y))
    def opposite(): Vector2d = new Vector2d(-x, -y)
